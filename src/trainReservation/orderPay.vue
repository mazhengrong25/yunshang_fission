          ...
          <view class="journey_card_top">
            <view class="card_top_item">
              <view class="top_item_date">{{
                detailData.segments[0]
                  ? $moment(detailData.segments[0].departure_time).format("MM-DD")
                  : "-"
              }}</view>
              <view class="top_item_date">{{
                detailData.segments[0]
                  ? $moment(detailData.segments[0].departure_time).format("ddd")
                  : "-"
              }}</view>
            </view>
            <view class="card_top_item">
              <view class="top_item_date">{{
                detailData.segments[0]
                  ? $moment(detailData.segments[0].arrive_time).format("MM-DD")
                  : "-"
              }}</view>
              <view class="top_item_date">{{
                detailData.segments[0]
                  ? $moment(detailData.segments[0].arrive_time).format("ddd")
                  : "-"
              }}</view>
            </view>
          </view>
          <view class="journey_card_bottom">
            <view class="bottom_left">
              <view class="left_time">{{
                detailData.segments[0]
                  ? $moment(detailData.segments[0].departure_time).format("hh:mm")
                  : "-"
              }}</view>
            ...
            <view class="bottom_center">
              <view class="bottom_center_item">
                {{
                  `${
                    detailData
                      ? Math.floor(Number(detailData.segments[0].travel_time) / 60)
                      : 0
                  }h${
                    detailData.segments[0].travel_time
                      ? Math.floor(Number(detailData.segments[0].travel_time) % 60)
                      : 0
                  }m`
                }}
              ...
              <image
                class="train_stop_image"
                src="@/static/train_stop.png"
                mode="contain"
              />
              <view class="bottom_center_item">{{
                detailData.segments[0].train_number
              }}</view>
            </view>
            <view class="bottom_right">
              <view class="left_time">{{
                detailData.segments[0]
                  ? $moment(detailData.segments[0].arrive_time).format("hh:mm")
                  : "-"
              }}</view>
...
export default {
  ...
  methods: {
    ...
    jumpPay() {
      ...
      let _that = this;
      train
        .payOrder(uni.arrayBufferToBase64(new Buffer(this.detailData.order_no)), data)
        .then((res) => {
          if (res.errorcode === 10000) {
            if (_that.payType === "微信") {
              ...
              uni.reLaunch({
                url:
                  "/flightReservation/payResult?orderData=" + JSON.stringify(orderInfo),
              });
            }
          } else {
            _that.payPayStatus = false;
