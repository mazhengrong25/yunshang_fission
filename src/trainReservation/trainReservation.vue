 ...
 * @LastEditTime: 2021-09-23 14:37:46
 * @LastEditors: mzr
-->
<template>
    <view class="train_reservation">
        ...
        <view class="reservation_background">
            <scroll-view :enable-back-to-top="true" :scroll-y="true" class="reservation_content" lower-threshold="100">

                <view class="cancel_rules">退改规则</view>
                <!-- 车次信息 -->
                <view class="train_box">
                    <trainMessageCard :trainObject="trainData" :isCabin="true" :seatObject="singleData" :noSeat="!isNOSeat()"  @getSeatStatus="getSeatStatus" :isPrice="true"></trainMessageCard>
                </view>
                <!-- 乘车人 -->
                <view class="passenger_box common_style">
                    <view class="passenger_top">
                        <view class="passenger_title">
                            ...
                            <text v-else>已选{{ passengerNumber.adt ? passengerNumber.adt + "成人" : ""}}{{ passengerNumber.chd ? "，" + passengerNumber.chd + "儿童" : ""}}</text>
                        </view>
                        <view class="passenger_button" @click="jumpPassenger()" v-if="!changeSubmitStatus">添加乘车人</view>
                    </view>

                    <uni-swipe-action class="passenger_main" v-if="passengerList.length > 0">
                        <uni-swipe-action-item :class="['main_list', { show: item.show }]" v-for="(item, index) in passengerList" :key="index">
                            <view class="remove_btn" @click="removePassenger(item)" v-if="!changeSubmitStatus"></view>
                                    ...
                                    <view class="message_info">
                                        <view class="bottom_left">
                                            <view class="bottom_title">
                                                <view v-if="item.type === '儿童'" class="title_child">用<view style="color: #FB9826">{{item.name}}</view>证件取票，身高低于1.5米</view>
                                                <view v-else>{{item.cert_type}}</view>
                                            </view>
                                            <view class="card">{{item.type === '儿童'?'':`${item.cert_no}`}}</view>
                                            <view class="child_explain" v-if="item.type === '儿童' && !changeSubmitStatus" @click="openChildDialog()">儿童票说明</view>

                                        </view>
                                    </view>
                                </view>
                                <view class="edit_btn" v-if="item.type !== '儿童' && !changeSubmitStatus"></view>
                                <view class="insurance_box" v-if="changeSubmitStatus">
                                    <yun-switch text="保险" @switchStauts="passInsSwitch()" :trainValue="item.is_insure" :index="index" :disabled="insuranceList.length < 1"></yun-switch>
                                </view>
                            </view>
                        ...
                        <view class="add_child_btn" @click="openAdultDialog()" v-if="!changeSubmitStatus">
                ...
                <view class="insurance_box_content common_style" v-if="changeSubmitStatus">
                    ...
                    <view class="insurance_box" v-if="insuranceList.length > 0">
                        <view class="insurance_box_item" v-for="(item, index) in insuranceList.slice(0, moreInsurance)" :key="index" @click="changeInsurance(insuranceList[index])">
                        ...
                        <view v-if="insuranceList.length > 4" @click="showMoreInsurance()" :class="['more_btn', { open: moreInsurance === 4 }]">{{ moreInsurance === 4 ? "展开全部" : "收起全部" }}</view>
                    </view>
                    <view class="insurance_box_no" v-else>暂无保险</view>
                </view>
                <!-- 选座服务 -->
                <view class="selection_service common_style" v-if="!isShowSeat() && !changeSubmitStatus">
                    <view class="selection_top">
                        <view class="top_title">选座服务</view>
                        <view class="top_action" 
                            v-if="trainData.train && (trainData.train.type === 'G' || trainData.train.type === 'D' || trainData.train.type === 'C') && !isShowSeat()">
                            可选座位：
                            <view :class="['top_action_first',{active:(this.getSeatNumber() === passengerList.length)}]">{{this.getSeatNumber()}}/</view>
                            <text>{{passengerList.length}}</text>
                        </view>
                    </view>

                    <!-- 座位 -->
                    <view v-if="trainData.train && (trainData.train.type === 'G' || trainData.train.type === 'D' || trainData.train.type === 'C')">
                        <view class="selection_siting">
                            <view class="seat_describe">靠窗</view>
                            <view class="seat_item">

                                <view :class="['seat_single',String(checkedCabinList).indexOf('1A') > -1?' active':'']" @click="checkedCabin('1A')">
                                    <view class="seat_number">A</view>
                                </view>

                                <view :class="['seat_single', String(checkedCabinList).indexOf('1B') > -1?' active':'']" @click="checkedCabin('1B')">
                                    <view class="seat_number">B</view>
                                </view>
                                <view :class="['seat_single', String(checkedCabinList).indexOf('1C') > -1?' active':'']" @click="checkedCabin('1C')">
                                    <view class="seat_number">C</view>
                                </view>
                            </view>
                            <view class="seat_describe">过道</view>
                            <view class="seat_item">
                                <view :class="['seat_single', String(checkedCabinList).indexOf('1D') > -1?' active':'']" @click="checkedCabin('1D')">
                                    <view class="seat_number">D</view>
                                </view>
                                <view :class="['seat_single', String(checkedCabinList).indexOf('1F') > -1?' active':'']" @click="checkedCabin('1F')">
                        ...
                        <view class="selection_siting" v-if="passengerList.length > 1">
                            ...
                            <view class="seat_item">
                                ...
                                <view :class="['seat_single', String(checkedCabinList).indexOf('2A') > -1?' active':'']" @click="checkedCabin('2A')">
                                    <view class="seat_number">A</view>
                                </view>

                                <view :class="['seat_single', String(checkedCabinList).indexOf('2B') > -1?' active':'']" @click="checkedCabin('2B')">
                                    <view class="seat_number">B</view>
                                </view>
                                <view :class="['seat_single', String(checkedCabinList).indexOf('2C') > -1?' active':'']" @click="checkedCabin('2C')">
                                    <view class="seat_number">C</view>
                                </view>
                            </view>
                            <view class="seat_describe">过道</view>
                            <view class="seat_item">
                                <view :class="['seat_single', String(checkedCabinList).indexOf('2D') > -1?' active':'']" @click="checkedCabin('2D')">
                                    <view class="seat_number">D</view>
                                </view>
                                <view :class="['seat_single', String(checkedCabinList).indexOf('2F') > -1?' active':'']" @click="checkedCabin('2F')">
                                    <view class="seat_number">F</view>
                                </view>
                            </view>
                            <view class="seat_describe">靠窗</view>
                ...
                <view class="order_contact common_style">
                    <view :class="['contact_title',{ noAsterisk: changeSubmitStatus}]">订单联系人信息</view>
                    <view class="contact_item_list">
                        <view class="contact_item">
                            <view class="item_title" :style="{ color: changeSubmitStatus ?'#999':''}">联系人</view>
                            
                            <view v-if="changeSubmitStatus" class="item_text">{{orderPassenger.name}}</view>
                            <input v-else   class="item_input" placeholder="请填写姓名" type="text" v-model="orderPassenger.name" placeholder-style="fontSize: 28upx;fontWeight:400;color:rgba(175,185,196,1);" />
                        </view>
                        <view class="contact_item">
                            <view class="item_title">手机号</view>
                            <view v-if="changeSubmitStatus" class="item_text">{{orderPassenger.phone}}</view>
                            <input v-else   class="item_input" placeholder="用于接收订单通知" placeholder-style="fontSize: 28upx;fontWeight:400;color:rgba(175,185,196,1);" type="number" v-model="orderPassenger.phone" />
                        </view>
                    </view>
                </view>
        ...
        <view class="bottom_bar">
            <view class="left_message">
                <view class="not_pass_message" v-if="passengerList.length < 1">请先添加乘车人</view>
            ...
            <view class="right_half">
                <view class="revise_Back" v-if="changeSubmitStatus" @click="reviseBack()">返回修改</view>
                <button :disabled="!trueNext" :class="['right_btn', { is_true: trueNext }]" type="default" @click="nextAction()" v-if="!changeSubmitStatus">
                    下一步
                </button>
                <button v-if="changeSubmitStatus" class="right_btn is_background" @click="submitOrder()">提交订单</button>
            </view>
        </view>
        ...
        <uni-popup ref="adultModal" type="bottom">
            <view class="adult_modal_box">
                ...
                <view class="modal_box_content">
                    <view :class="['cert_box',{active:item.name === checkedAdtPassenger.name }]" v-for="(item,index) in passengerList" :key="index" @click="checkedCard(item)" v-if="item.type !== '儿童'">
                        <view class="box_title">{{item.name}}</view>
                        <view class="box_input">{{item.cert_type}}尾号{{item.cert_no?(item.cert_no).slice(-4):'-'}}</view>
                    </view>
                </view>
            </view>
        </uni-popup>
                ...
                <view class="modal_box_content">
                    <view class="price_box">
                        ...
                        <view class="fare_item">
                            ...
                            <view class="item_box">
                                <view class="item_box_price">&yen;{{insuranceActive.default_dis_price || 0}}</view>
    ...
    methods: {
        ...
        jumpPassenger() {
            uni.navigateTo({
                url:
                    ...
                    "&editPassengerList=" +
                    JSON.stringify(this.passengerList) + '&trainType=train',
            });
        },
        ...
        checkedCabin(val) {
            ...
            if (String(this.checkedCabinList).indexOf(val) < 0) {
                ...
                this.checkedCabinList.splice(this.checkedCabinList.findIndex(item => item === val), 1)
            }
        ...
        getOrderPrice() {
            ...
            totalPrice =
                ...
                Number(this.insuranceActive.default_dis_price || 0) * Number(this.insuranceNumber)
        ...
        // 下一步
        nextAction() {
            if((this.trainData.train.type === 'G' || this.trainData.train.type === 'D' || this.trainData.train.type === 'C') && this.passengerList.length !== this.checkedCabinList.length) {
        ...
        submitOrder() {
            ...
            this.passengerList.map(item => {
                passenger.push({
                    ...
                    ticket_type: item.type === "成人" ? "1" : item.type === "儿童" ? "2" : item.type,                //类型：String  必有字段  备注：票类型
                    phone: item.phone || "",                //类型：String  必有字段  备注：手机号
                    is_insurance: item.is_insure?true:false,                //类型：String  必有字段  备注：是否购买保险
                    school: {                //类型：Object  可有字段  备注：学校信息，默认为空。如果是学生票则必须
                    ...
                    passport: {                //类型：Object  可有字段  备注：护照信息，如果是护照则必须
            ...
            let data = {
                ...
                train: {
                    ...
                    departure_date: `${this.$moment(this.trainData.train.departure_date).format('YYYY-MM-DD')} ${this.trainData.train.departure}`, //类型：String  必有字段  备注：出发日期
                    arrive_date: `${this.$moment(this.trainData.train.departure_date).add(this.trainData.train.days, 'd').format('YYYY-MM-DD')} ${this.trainData.train.arrive}`, //类型：String  必有字段  备注：到达日期
        ...
        changeInsurance(val) {
            ...
            this.insuranceActive = val.id !== this.insuranceActive.id ? val : val.id === this.insuranceActive.id ? {} : {};
        ...
        .passenger_box {
            ...
            .passenger_main {
                ...
                .main_list {
                    ...
                    .remove_btn {
                        background: url(@/static/number_remove_btn.png)
                    ...
                    .list_message {
                                ...
                                .bottom_left {
                                    ...
                                    .child_explain {
                                        ...
                                        &::after {
                                            ...
                                            top: 9upx;
                                            left: 125upx;
                                            background: url("@/static/train_arrow.png")
                                                no-repeat center center;
                                            background-size: contain;
                                        }
                                    }
                        ...
                        .edit_btn {
                            background: url(@/static/edit_icon.png) no-repeat
                                center center;
                            background-size: contain;
                            width: 30upx;
                            height: 30upx;
                ...
                .insurance_box_item {
                    ...
                    .item_name {
                        ...
                        .item_icon {
                            ...
                            width: 47upx;
                            height: 33upx;
                            background: url(@/static/insurance_type_1.png)
                                no-repeat center center;
                            background-size: contain;
                            margin-left: 10upx;
                            &.true {
                                background: url(@/static/insurance_type_2.png)
                                    no-repeat center center;
                                background-size: contain;
                            }
                        }
            ...
            .selection_siting {
                ...
                .seat_item {
                    ...
                    .seat_single {
                        width: 60upx;
                        height: 64upx;
                        background: url("@/static/train_seat.png") no-repeat
                            center center;
                        background-size: contain;
                        position: relative;
                        .seat_number {
                        ...
                        &.active {
                            background: url("@/static/train_active_seat.png")
                                no-repeat center center;
                            background-size: contain;
            ...
            .pass_message {
                .pass_message_price {
                    ...
                    &::after {
                        content: "";
                        display: inline-block;
                        background: url(@/static/flight_checked_btn.png)
                            no-repeat center center;
                        background-size: contain;
                        width: 16upx;
                        height: 12upx;
