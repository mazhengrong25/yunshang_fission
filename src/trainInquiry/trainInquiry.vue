...
<template>
    <view class="train_inquery">
        ...
        <view class="train_header">
            <scroll-view scroll-x="true" class="train_time_list" :scroll-into-view="timeIndex">
                <blticket_priceock v-for="(item, index) in ticketTimeList" :key="index">
                    <view :id="item.id" :class="['train_time_btn',{'active': item.date === activeTimeNumber},{'is_before': !item.status}]" @click="clickBtn(item,item.status)">
        ...
        <scroll-view :enable-back-to-top="true" :scroll-y="true" class="train_content" lower-threshold="100">
            <!-- 正常车次 -->
            <view class="train_list" v-for="(item,index) in saleList" :key="index" @click="jumpTrainInfo(item)">
                    ...
                    <view class="item_left">
                        ...
                        <view class="item_left_join">
                            <view class="left_join_time">
                                ...
                                )}h${Math.floor(Number(item.train.run_minute) % 60)}m`}}
                            </view>
                ...
                <view class="train_bottom">
                    <view v-if="seatStatus(item.seat)">
                        <view class="train_bottom_item" :style="{display:(ticketStatus && pitem.number < 1) ? 'none':''}" v-for="(pitem,pindex) in item.seat" :key="pindex">
                            <view class="train_seat_title" :style="{ color: pitem.number < 1 ?'#C5C5C5':''}">{{pitem.name}}:</view>
                            <view class="train_seat_value" :style=" { color: pitem.number <= 15 && pitem.number > 0?'#FB9826':pitem.number < 1?'#C5C5C5':'' }">
                                {{pitem.number < 1 ? '无': pitem.number > 15 ? '有': pitem.number}}
                            </view>
                        </view>
                    </view>
                    <view v-else class="train_bottom_notify">暂无余票，请选择其他车次或稍后再试</view>
                </view>
            </view>

            <!-- 折叠栏 -->
            <view :class="['train_fold',{active:folderStatus}]" v-if="trainList.length > 1 && noSaleList.length > 1">
                <span>{{folderStatus ? `以下为${noSaleList.length}条暂停发售/未开售车次` : `已为您折叠${noSaleList.length}条暂停发售车次`}}</span>
                <view class="train_action" :style="{display:(folderStatus === false ? '':'none')}" @click="openFolder()">查看</view>
            </view>

            <!-- 未开售车次 -->
            <view class="train_list" v-for="(item,index) in folderStatus?noSaleList:[]" :key="index">
                <view class="train_top">
                    <view class="item_left">
                        <view class="item_left_section">
                            <view class="section_time">{{item.train.departure}}</view>
            ...
            <!-- 骨架屏 -->
            <view class="flight_skeleton" v-for="i in trainList.length < 1? skeletonNumber :0" :key="i">
                <view class="top">
                    <text></text>
                    <text></text>
                </view>
        ...
        <!-- 筛选弹窗 -->
        <filter-Dialog ref="filterDialog" :trainType="trainTypeList" :departureStation="departureStationList" :arriveStation="arriveStationList" @trainFilterData="trainFilter"></filter-Dialog>
    </view>
</template>
...
export default {
    ...
    methods: {
        ...
        jumpTrainInfo(val) {
            uni.navigateTo({
                url: "/pages/trainInfo/trainInfo?trainData=" +
        ...
        trainFilter(e, val) {
            ...
            if (e[1]) {
                if (e[1] === "不限") {
                    ...
                    let newData = []
                    this.trainList.forEach(item => {
                        let thisDate = this.$moment().format('YYYY-MM-DD') + ' ' + item.train.departure  // 拿到当前列表时间
                        // 当前时间 比较  其他规定两个时刻
                        if (e[1] === '0-6点') {
                            if (this.$moment(thisDate).isBetween(this.$moment().format('YYYY-MM-DD') + ' 00:00', this.$moment().format('YYYY-MM-DD') + ' 06:00')) {
                                newData.push(item)
                            }

                        }
                        if (e[1] === '6-12点') {
                            if (this.$moment(thisDate).isBetween(this.$moment().format('YYYY-MM-DD') + ' 06:00', this.$moment().format('YYYY-MM-DD') + ' 12:00')) {
                                newData.push(item)
                            }

                        }
                        if (e[1] === '12-18点') {
                            if (this.$moment(thisDate).isBetween(this.$moment().format('YYYY-MM-DD') + ' 12:00', this.$moment().format('YYYY-MM-DD') + ' 18:00')) {
                                newData.push(item)
                            }

                        }
                        if (e[1] === '18-24点') {
                            if (this.$moment(thisDate).isBetween(this.$moment().format('YYYY-MM-DD') + ' 18:00', this.$moment().format('YYYY-MM-DD') + ' 24:00')) {
                                newData.push(item)
                            }
        ...
        filterList() {
            ...
            if (this.checkboxStatus) {
                ...
                if (this.checkboxStatusIndex !== null) {
                    this.trainFilter([this.trainTypeList[this.checkboxStatusIndex]], this.checkboxStatusIndex)
                }
            }
        ...
        getDateList() {
            ...
            for (let index = 0; index < 14; index++) {
                this.ticketTimeList.push({
                    id: "id_" + index,
                    day: this.$moment(day).add(dayNumber, "d").calendar(null, {
                        lastDay: "昨天",
                        sameDay: "今天",
                        nextDay: "明天",
                        nextWeek: "ddd",
                    ...
                    date: this.$moment(this.trainData.toTime.date)
                        ...
                        .format("YYYY-MM-DD"),
                    number:
                        this.$moment(day).add(dayNumber, "d").format("DD") === "01"
                            ? this.$moment(day).add(dayNumber, "d").format("MM") +
                            "-" +
                            this.$moment(day).add(dayNumber, "d").format("DD")
                            : this.$moment(day).add(dayNumber, "d").format("DD"),
                    status: this.$moment(day).add(dayNumber, "d").format('YYYY-MM-DD') === this.$moment(+this.$moment()).format('YYYY-MM-DD') ? true : +this.$moment(day).add(dayNumber, "d") > +this.$moment(),
                });
                dayNumber += 1;
            }
            this.$nextTick(() => {
    ...
    .train_content {
        ...
        .train_fold {
            ...
            .train_action {
                ...
                &::after {
                    content: "";
                    background: url("@/static/train_find.png") no-repeat center
                        center;
                    width: 20upx;
                    height: 10upx;
