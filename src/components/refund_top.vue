<template>
    <view class="refund_top">
        ...
        <!-- 退票理由选择 -->
        <yun-selector ref="groupPopup" :dataList="reasonGroup" @submitDialog="groupPopupSelecctBtn()"></yun-selector>
        <!-- 改签原因选择 -->
        <yun-selector ref="changeGroupPopup" :dataList="causeGroup" @submitDialog="changeGroupPopupSelecctBtn()"></yun-selector>
    </view>
</template>

<script>
import reasonApi from "@/api/notice.js";
export default {
    components: {},
    props: {
        dataList: {
    ...
    methods: {
        ...
        getReasonList() {
            ...
            reasonApi.getReason().then((res) => {
                if (res.errorcode === 10000) {
                    this.reasonGroup = res.data.refund
                    for (const key in res.data.change) {
                        this.causeGroup.push(key)
                    }
