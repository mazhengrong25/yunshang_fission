    ...
    <view class="train_refund">
        ...
        <view class="order_filter">
            ...
            <view 
              :class="[
                'filter_list',
                { active: JSON.stringify(this.refundListFilter) !== '{}' },
              ]"
              @click="goFilter('1')">
                <view class="list_icon">
                    <image
                      v-if="JSON.stringify(this.refundListFilter) !== '{}'"
        ...
        <scroll-view :scroll-y="true" :enable-back-to-top="true" :scroll-top="scrollTop" @scroll="scroll" class="content">
          <view
            ...
            :key="index"
          >
            <view class="list_item" @click="jumpRefundDetails(item)">
              <view class="item_header">
                <view class="item_title">
                  <view class="title">{{ item.segments[0].from_city }} - {{ item.segments[0].to_city }}</view>
                </view>
                <view class="info_right">
                  <text :style="{color:(item.status === 5)?'#FF0000':''}">
                    {{
                      item.status === 1
                        ? "退票中"
                        : item.status === 2
...
export default {
    data() {
      return {
        iStatusBarHeight: 0,
        oldScrollTop: 0,
        orderHeaderTitle: "火车票退票订单",
        headerList: ["全部", "退票中", "已退票", "退票失败"], // 订单列表类别
        headerActive: 0,
    ...
    methods: {
        ...
        checkedHeaderActive(index) {
            ...
            this.getTrainRefundList();
        },

        // 时间排序
        sorTime() {
          this.applyTimeStatus = !this.applyTimeStatus
          this.refundList.sort(this.applySort("created_at"));
        },
        ...
        getTrainRefundList() {
          ...
          orderApi.trainRefundList(data).then((res) => {
            if (res.code === 0) {
              ...
              // 缺省状态
              this.showDefault = this.refundList.length < 1;
            } else {
        ...
        // 跳转到详情
        jumpRefundDetails(data) {
            uni.navigateTo({
                url:
                    "/order/trainRefundDetails?refund_no=" +
                    data.refund_no
            });
        },
    ...
    onShow() {
      this.refundListFilter = uni.getStorageSync("trainFilter")
        ? JSON.parse(uni.getStorageSync("trainFilter"))
        : {};
      if(JSON.stringify(this.refundListFilter) !== "{}") {
        //退票状态筛选
        if (this.refundListFilter.status !== null) {
          this.checkedHeaderActive(Number(this.refundListFilter.status));
        } else {
