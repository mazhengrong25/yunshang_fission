...
<template>
    <view class="train_order">
        ...
        <view class="order_filter">
            <view :class="['filter_list', { active: sortType === 'reserve' }]" @click="sorTime('reserve')">
                <view class="list_icon">
                    <image v-if="sortType === 'reserve'" src="@/static/filter_time_active.png" mode="contain" />
                    <image v-else src="@/static/filter_time.png" mode="contain" />
                </view>
                <view class="list_title">预定(早-晚)</view>
            </view>

            <view :class="['filter_list', { active: sortType === 'depart' }]" @click="sorTime('depart')">
                <view class="list_icon">
                    <image v-if="sortType === 'depart'" src="@/static/filter_setoff_active.png" mode="contain" />
        ...
        <scroll-view :scroll-y="true" :enable-back-to-top="true" :scroll-top="scrollTop" @scroll="scroll" class="content">
            ...
            <view class="content_list" 
                ...
                v-for="(item,index) in trainList" :key="index">
                <view class="list_item" @click="jumpTrainDetails(item)">
                    <view class="item_header">
                        <view class="item_title">
                            <view class="title">
                                {{item.segments[0].from_city}} -
                                {{item.segments[0].to_city}}</view>
                        </view>
                    ...
                    <view class="item_info">
                        <view class="info_left">
                            ...
                            <text>{{item.segments[0].departure_time?$moment(item.segments[0].departure_time).format("MM月DD日"):'-'}}</text>
                            <text>{{item.segments[0].departure_time?$moment(item.segments[0].departure_time).format("HH:mm"):'-'}}发车</text>
                        </view>
                    ...
                    <view class="passenger_item">
                        <view class="item_title" v-for="(oitem, oindex) in item.passengers" :key="oindex">
