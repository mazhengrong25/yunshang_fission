          ...
          >
            <view class="list_item" @click="jumpChangeDetails(item)">
              ...
              <view class="item_time" v-if="item.status === 2">
                  ...
                  <view class="time_number">{{ Math.floor(($moment(item.created_at).add(10,'m').diff($moment(),'s') / 60) % 60) }}分钟</view>
              </view>
    ...
    methods: {
        ...
        getTrainChangeList() {
          let data = {
            ...
            // train_date_start: this.changeListFilter.train_date_start || '', // 开始时间
